...
with col_header:
    st.markdown('<div class="main-header">Manufacturing Defect Analysis Tool</div>',
                unsafe_allow_html=True)

# --- Sidebar Configuration ---
st.sidebar.header("Configuration")
uploaded_file = st.sidebar.file_uploader(
    "Upload Manufacturing Data (CSV)", type="csv")

# Developer Option: Load Sample Data
...
if os.path.exists("Sample.csv"):
    if st.sidebar.button("Load Sample Data (Sample.csv)"):
...
if uploaded_file is not None:
    try:
        ...
        # Verify columns
        required_cols = ['line_description', 'bde_order_number',
                         'GCAS', 'counter_type_description', 'count']
        ...
        if 'DATE' in df.columns:
            ...
            st.warning(
                "Dataset missing 'DATE' column. Date filtering disabled.")
        ...
        # 1.1 GCAS Filter
        all_gcas = sorted(df['GCAS'].dropna().unique().astype(str))
        selected_gcas = st.sidebar.multiselect(
            "Select GCAS",
            options=all_gcas,
            default=all_gcas,
            help="Filter data by specific GCAS codes. If none selected, data will be empty."
        )
        if selected_gcas:
            df = df[df['GCAS'].astype(str).isin(selected_gcas)]
        else:
            # If nothing is selected, we should probably show nothing or a warning later.
            # But for now, let's keep it as is. User requested "allow user to filter".
            # If they deselect all, it means they want to see nothing of those GCAS.
            df = df[df['GCAS'].astype(str).isin([])]
        ...
        if not good_parts_selection:
            st.warning(
                "Please select at least one counter type representing 'Good Parts' to proceed.")
            st.stop()

        # --- Logic & Calculations: Identification ---
        # 1. Identify Defects based on User Input Pattern
        # Case insensitive check
        df['is_defect'] = df['counter_type_description'].astype(
            str).str.strip().str.lower().str.startswith(defect_pattern.lower())
        df['is_good'] = df['counter_type_description'].isin(
            good_parts_selection)

        # 2. Extract and Normalize Visual Defects for Filtering
        # We process the descriptions to create a clean list for the filter

        # Create a temporary column for normalization
        df['normalized_desc_filter'] = df['counter_type_description'].astype(
            str)

        # Only normalize rows that are defects
        defect_mask = df['is_defect']
        if defect_mask.any():
            # Apply normalization comparable to Section 4
            df.loc[defect_mask, 'normalized_desc_filter'] = df.loc[defect_mask,
                                                                   'normalized_desc_filter'].str.replace(r'operator\s*[0-9]+', '', case=False, regex=True)
            df.loc[defect_mask, 'normalized_desc_filter'] = df.loc[defect_mask, 'normalized_desc_filter'].str.replace(
                r'hand written errors', '', case=False, regex=True)
            df.loc[defect_mask, 'normalized_desc_filter'] = df.loc[defect_mask,
                                                                   'normalized_desc_filter'].str.strip()
            df.loc[defect_mask, 'normalized_desc_filter'] = df.loc[defect_mask,
                                                                   'normalized_desc_filter'].str.replace(r'\s+', ' ', regex=True)

            # Get unique visual defects
            unique_visual_defects = sorted(
                df.loc[defect_mask, 'normalized_desc_filter'].unique())

            # 3. Visual Defect Filter
            selected_visual_defects = st.sidebar.multiselect(
                "Select Visual Defects",
                options=unique_visual_defects,
                default=unique_visual_defects,
                help="Filter the analysis to specific defect types found in 'Hand Written Defect Analysis'."
            )

            # Filter Dataframe: Keep row IF (It is Good Part) OR (It is a Defect AND its normalized name is selected)
            # This effectively filters OUT defects that are not selected, updating the PPM calculation.
            # We keep non-defect/non-good rows (if any exist) to be safe, or just stick to the definition.
            # Let's preserve rows that are NOT defects to be safe (Good parts + others), and filter ONLY defects.

            df = df[(~df['is_defect']) | (df['is_defect'] &
                                          df['normalized_desc_filter'].isin(selected_visual_defects))]
        ...
        # --- KPI Overview ---
        st.markdown('<div class="sub-header">Overview</div>',
                    unsafe_allow_html=True)
        ...
        with col3:
            ...
            overall_ppm = (total_defect / (total_good + total_defect)
                           * 1_000_000) if (total_good + total_defect) > 0 else 0
            st.metric("Overall PPM", f"{overall_ppm:.0f}")

        # --- Graph 1: Gaussian Distribution per Line (Aggregated by Order) ---
        st.markdown('---')
        st.markdown(
            '<div class="sub-header">1. Order Performance Distribution (Gaussian/KDE)</div>', unsafe_allow_html=True)

        selected_line = st.selectbox(
            "Select Production Line", df['line_description'].unique())
        ...
        if not df_order_ppm.empty and df_order_ppm['ppm'].sum() > 0:
            ...
            # Filter for Plotting
            filtered_plot_data = df_order_ppm[df_order_ppm['ppm']
                                              <= max_ppm_limit]
            ...
            with col_a:
                ...
                with plt.style.context(plt_style):
                    ...
                    if not filtered_plot_data.empty:
                        # Histogram
                        sns.histplot(filtered_plot_data['ppm'], kde=False, ax=ax,
                                     color='skyblue', stat='percent', label='% of Orders')
                        ...
                        y_vals = 100 * \
                            (1 - np.arange(len(sorted_ppm_all)) / len(sorted_ppm_all))
                        ...
                        ax2 = ax.twinx()
                        ax2.plot(x_plot, y_plot, color='darkblue',
                                 linewidth=2, label='Reverse Cumulative % (> X PPM)')
                        ...
                        st.warning(
                            f"No data points below {max_ppm_limit} PPM.")
                    ...
                    # Plot customization
                    ax.axvline(2500, color='orange', linestyle='--',
                               linewidth=2, label='Limit: 2500 PPM')
                    ax.axvline(5000, color='red', linestyle='--',
                               linewidth=2, label='Limit: 5000 PPM')

                    # Add Interval Percentage Lines at the Top
                    pct_0_2500 = (df_order_ppm['ppm'] <= 2500).mean() * 100
                    pct_2500_5000 = ((df_order_ppm['ppm'] > 2500) & (
                        df_order_ppm['ppm'] <= 5000)).mean() * 100
                    ...
                    # Line 0-2500
                    ax.plot([0, 2500], [y_line_pos, y_line_pos],
                            color='grey', linestyle=':', linewidth=1)
                    ax.text(1250, y_line_pos, f"{pct_0_2500:.1f}%", ha='center',
                            va='bottom', fontsize=9, color=label_color, fontweight='bold')

                    # Line 2500-5000
                    ax.plot([2500, 5000], [y_line_pos, y_line_pos],
                            color='grey', linestyle=':', linewidth=1)
                    ax.text(3750, y_line_pos, f"{pct_2500_5000:.1f}%", ha='center',
                            va='bottom', fontsize=9, color=label_color, fontweight='bold')

                    # Line > 5000
                    if max_ppm_limit > 5000:
                        ax.plot([5000, max_ppm_limit], [
                                y_line_pos, y_line_pos], color='grey', linestyle=':', linewidth=1)
                        mid_point = 5000 + (max_ppm_limit - 5000) / 2
                        ax.text(mid_point, y_line_pos, f"{pct_gt_5000:.1f}%", ha='center',
                                va='bottom', fontsize=9, color=label_color, fontweight='bold')

                    ax.set_title(
                        f"PPM Distribution - {selected_line}", fontsize=14)
                    ...
                    ) if not filtered_plot_data.empty else ([], []))
                    legend = ax.legend(lines_1 + lines_2, labels_1 + labels_2, loc='upper right',
                                       bbox_to_anchor=(1, 0.85), facecolor=app_bg_color, edgecolor=text_color)
            ...
            st.info(
                "No sufficient data to plot distribution (PPM might be 0 for all orders).")

        # --- High PPM Orders Table (Requested Update) ---
        col_hdr, col_inp = st.columns([3, 1])
        with col_inp:
            ppm_threshold_orders = st.number_input(
                "PPM Threshold",
                min_value=0,
                max_value=int(max_ppm_limit),
                value=2500,
                step=100,
                key="ppm_thresh_orders"
            )
        with col_hdr:
            st.markdown(f'#### Orders > {ppm_threshold_orders} PPM')

        # Filter orders > threshold using the same data as the graph (selected line)

        high_ppm_orders = []
        for order_id, group in order_groups:
            ppm = calculate_ppm(group)
            if ppm > ppm_threshold_orders:
        ...
        if high_ppm_orders:
            df_high_ppm = pd.DataFrame(
                high_ppm_orders).sort_values('PPM', ascending=False)

            # View Toggle
            order_view_format = st.radio("View Format", [
                                         "Table", "Bar Graph"], horizontal=True, key="high_ppm_view_format")

            if order_view_format == "Table":
                # Style High PPM Table
                st.dataframe(
                    df_high_ppm.style.format(
                        {'PPM': "{:.0f}", 'Gross Parts (Good)': "{:.0f}"})
                ...
                df_high_ppm['Order Labels'] = df_high_ppm['Order Number'].astype(
                    str)

                order_chart = alt.Chart(df_high_ppm).mark_bar().encode(
                    x=alt.X('PPM', title='PPM'),
                    y=alt.Y('Order Labels', sort='-x', title='Order Number',
                            axis=alt.Axis(labelLimit=1000)),
                    tooltip=['Order Number', alt.Tooltip('PPM', format=',.0f'), alt.Tooltip(
                        'Gross Parts (Good)', format=',.0f')]
                ).properties(
                ...
                try:
                    png_order = vlc.vegalite_to_png(
                        order_chart.to_json(), scale=2)
                    st.download_button("Download Graph as PNG", png_order,
                                       "high_ppm_orders_graph.png", "image/png", key='dl_img_order')
        ...
        st.markdown('---')
        st.markdown(
            '<div class="sub-header">2. GCAS Performance: PPM vs Volume</div>', unsafe_allow_html=True)

        # Line Filter for Scatter Graph
        col_scatter_1, col_scatter_2, col_scatter_3 = st.columns([2, 1, 1])
        with col_scatter_1:
            graph_line_option = st.selectbox("Select Line for Graph", [
                                             "All Lines"] + list(df['line_description'].unique()))
        ...
        if not df_scatter.empty:
            ...
            if not df_scatter.empty:
                ...
                # Threshold Inputs
                with col_scatter_2:
                    vol_threshold = st.number_input("Volume Threshold (X)", min_value=0, max_value=int(
                        max_vol*1.1), value=default_vol_thresh, step=100)
                with col_scatter_3:
                    ppm_threshold = st.number_input("PPM Threshold (Y)", min_value=0, max_value=int(
                        max_ppm_limit), value=2500, step=50)
                ...
                for q in ["Q1 (Critical)", "Q2 (Sporadic)", "Q3 (Small Run)", "Q4 (Good)"]:
                    ...
                    stats.append({
                        "Quadrant": q,
                        "% GCAS": (n_gcas / total_gcas) if total_gcas > 0 else 0,
                        "% Volume": (vol_gcas / total_volume) if total_volume > 0 else 0,
                        "Count": n_gcas
                    })

                # Plot Base
                base = alt.Chart(df_scatter).encode(
                    x=alt.X('Total Good Parts',
                            title='Total Good Parts (Volume)'),
                    y=alt.Y('PPM', title='Defect Rate (PPM)'),
                    tooltip=['GCAS', 'Description', 'PPM',
                             'Total Good Parts', 'Quadrant']
                )

                # Points
                points = base.mark_circle(size=60).encode(
                    color=alt.Color('Quadrant', scale=alt.Scale(domain=["Q1 (Critical)", "Q2 (Sporadic)", "Q3 (Small Run)", "Q4 (Good)"], range=[
                                    'red', 'orange', 'lightgrey', 'green']), legend=None)
                ).interactive()

                # Rule Lines
                h_rule = alt.Chart(pd.DataFrame({'y': [ppm_threshold]})).mark_rule(
                    color='black', strokeDash=[5, 5]).encode(y='y')
                v_rule = alt.Chart(pd.DataFrame({'x': [vol_threshold]})).mark_rule(
                    color='black', strokeDash=[5, 5]).encode(x='x')

                # Altair Theme Enforcement
                chart = (
                    points + h_rule + v_rule).properties(height=500).configure(background=app_bg_color)
                ...
                try:
                    ...
                    st.download_button("Download Scatter Plot as PNG", png_scatter,
                                       "gcas_scatter.png", "image/png", key='dl_img_scatter')
                except Exception as e:
                    # Fallback or silent fail if needed, but error is better for debug
                    st.caption(f"Image download unavailable: {e}")

                # Display Stats as Donut Charts
                st.markdown("#### Quadrant Analysis Distribution")

                df_stats = pd.DataFrame(stats)

                # Common Color Scale
                domain = ["Q1 (Critical)", "Q2 (Sporadic)",
                          "Q3 (Small Run)", "Q4 (Good)"]
                range_colors = ['red', 'orange', 'lightgrey', 'green']

                # Stacked Bar Chart Base
                base_bar = alt.Chart(df_stats).mark_bar().encode(
                    color=alt.Color('Quadrant', scale=alt.Scale(
                        domain=domain, range=range_colors)),
                    tooltip=['Quadrant', alt.Tooltip('% GCAS', format='.1%'), alt.Tooltip(
                        '% Volume', format='.1%'), 'Count']
                ).properties(height=100)  # Defined height for bars

                # Chart 1: % GCAS Stacked Bar
                bar_gcas = base_bar.encode(
                    x=alt.X('% GCAS', title='% GCAS',
                            axis=alt.Axis(format='%')),
                    order=alt.Order('Quadrant', sort='ascending')
                ).properties(title="Distribution by GCAS Count")

                # Chart 2: % Volume Stacked Bar
                bar_vol = base_bar.encode(
                    x=alt.X('% Volume', title='% Volume',
                            axis=alt.Axis(format='%')),
                    order=alt.Order('Quadrant', sort='ascending')
                ).properties(title="Distribution by Production Volume")

                # Concatenate
                h_chart = alt.hconcat(
                    bar_gcas, bar_vol).resolve_scale(color='shared')

                # Apply theme
                h_chart = h_chart.configure_view(
                    strokeWidth=0).configure_title(fontSize=14)

                if theme_mode:
                    h_chart = h_chart.configure_legend(
                        labelColor='white', titleColor='white'
                    ).configure_title(color='white')
                else:
                    h_chart = h_chart.configure_legend(
        ...
        col_gcas_hdr, col_gcas_inp = st.columns([3, 1])
        with col_gcas_inp:
            ppm_threshold_gcas = st.number_input(
                "PPM Threshold",
                min_value=0,
                max_value=int(max_ppm_limit),
                value=2500,
        ...
        with col_gcas_hdr:
            st.markdown(
                f'<div class="sub-header">Top GCAS Offenders (> {ppm_threshold_gcas} PPM)</div>', unsafe_allow_html=True)

        # Filter for Table
        table_line_option = st.selectbox("Select Line for Top Offenders Table", [
                                         "All Lines"] + list(df['line_description'].unique()))
        ...
        df_gcas_table = pd.DataFrame(
            gcas_table_data).sort_values('PPM', ascending=False)

        offenders = df_gcas_table[df_gcas_table['PPM'] > ppm_threshold_gcas]

        if not offenders.empty:
            st.info(
                f"Showing {len(offenders)} offenders for {table_line_option}")

            # View Toggle
            gcas_view_format = st.radio("View Format", [
                                        "Table", "Bar Graph"], horizontal=True, key="gcas_offenders_view_format")

            if gcas_view_format == "Table":
                st.dataframe(
                    offenders.style.format(
                        {'PPM': "{:.0f}", 'Total Good Parts': "{:.0f}"})
                    .set_properties(**{'background-color': app_bg_color, 'color': app_text_color}),
                    use_container_width=True,
                    hide_index=True
                )
                ...
                offenders['Label'] = offenders['GCAS'].astype(
                    str) + " - " + offenders['Description'].astype(str)

                gcas_chart = alt.Chart(offenders).mark_bar().encode(
                    x=alt.X('PPM', title='PPM'),
                    y=alt.Y('Label', sort='-x', title='GCAS - Description',
                            axis=alt.Axis(labelLimit=1000)),
                    tooltip=['GCAS', 'Description', alt.Tooltip(
                        'PPM', format=',.0f'), alt.Tooltip('Total Good Parts', format=',.0f')]
                ).properties(
                ...
                try:
                    png_gcas = vlc.vegalite_to_png(
                        gcas_chart.to_json(), scale=2)
                    st.download_button("Download Graph as PNG", png_gcas,
                                       "gcas_offenders_graph.png", "image/png", key='dl_img_gcas')
                except Exception as e:
                    st.error(f"Could not generate image: {e}")
        ...
        st.markdown('---')
        st.markdown(
            '<div class="sub-header">3. Weekly PPM Trend</div>', unsafe_allow_html=True)

        # Line Filter for Weekly Graph
        weekly_line_option = st.selectbox("Select Line for Trend Graph", [
                                          "All Lines"] + list(df['line_description'].unique()), key='weekly_filter')

        if weekly_line_option == "All Lines":
            weekly_data_source = df
        else:
            weekly_data_source = df[df['line_description']
                                    == weekly_line_option]

        if 'DATE' in weekly_data_source.columns:
            weekly_data_source['Week_Num'] = weekly_data_source['DATE'].dt.isocalendar(
            ).week
            ...
            if not df_weekly.empty:
                ...
                # Limit Lines
                limit_2500 = alt.Chart(pd.DataFrame({'y': [2500]})).mark_rule(
                    color='orange', strokeDash=[5, 5], strokeWidth=2).encode(y='y')
                limit_5000 = alt.Chart(pd.DataFrame({'y': [5000]})).mark_rule(
                    color='red', strokeDash=[5, 5], strokeWidth=2).encode(y='y')
                ...
                try:
                    png_trend = vlc.vegalite_to_png(
                        final_chart.to_json(), scale=2)
                    st.download_button("Download Trend Graph as PNG", png_trend,
                                       "weekly_trend.png", "image/png", key='dl_img_trend')
                except Exception as e:
                    st.caption(f"Image download unavailable: {e}")
        ...
        st.markdown('---')
        st.markdown(
            '<div class="sub-header">4. Hand Written Defect Analysis</div>', unsafe_allow_html=True)

        # Filter
        defects_line_option = st.selectbox("Select Line for Defect Analysis", [
                                           "All Lines"] + list(df['line_description'].unique()), key='defects_filter')

        if defects_line_option == "All Lines":
            defects_data_source = df
        else:
            defects_data_source = df[df['line_description']
                                     == defects_line_option]
        ...
        if not defect_rows.empty and total_good_parts_scope > 0:
            ...
            defect_analysis_df['normalized_desc'] = defect_analysis_df['counter_type_description'].astype(
                str).str.replace(r'operator\s*[0-9]+', '', case=False, regex=True)

            # Remove "Hand written errors" (case insensitive)
            defect_analysis_df['normalized_desc'] = defect_analysis_df['normalized_desc'].str.replace(
                r'hand written errors', '', case=False, regex=True)
            ...
            defect_groups = defect_analysis_df.groupby(
                'normalized_desc')['count'].sum().reset_index()
            ...
            total_parts_scope = total_good_parts_scope + \
                defect_rows['count'].sum()

            defect_groups['PPM Impact'] = (
                defect_groups['count'] / total_parts_scope) * 1_000_000

            # Rename for display
            defect_groups = defect_groups.rename(
                columns={'normalized_desc': 'Defect Description', 'count': 'Defect Count'})

            # Sort
            defect_groups = defect_groups.sort_values(
                'PPM Impact', ascending=False)

            st.info(f"Showing defect breakdown for {defects_line_option}")

            # View Toggle
            view_format = st.radio("View Format", [
                                   "Table", "Bar Graph"], horizontal=True, key="defect_view_format")

            if view_format == "Table":
                st.dataframe(
                    defect_groups.style.format(
                        {'PPM Impact': "{:.0f}", 'Defect Count': "{:.0f}"})
                    .set_properties(**{'background-color': app_bg_color, 'color': app_text_color}),
                    use_container_width=True,
                    hide_index=True
                )
                ...
                bar_chart = alt.Chart(defect_groups).mark_bar().encode(
                    ...
                    y=alt.Y('Defect Description', sort='-x',
                            title='Defect Description', axis=alt.Axis(labelLimit=1000)),
                    tooltip=['Defect Description', 'Defect Count',
                             alt.Tooltip('PPM Impact', format=',.0f')]
                ).properties(
                    title=f"Hand Written Defect Impact ({defects_line_option})",
                    # Dynamic height based on number of bars
                    height=max(400, len(defect_groups) * 30)
                ).configure(background=app_bg_color)
                ...
                try:
                    png_defect = vlc.vegalite_to_png(
                        bar_chart.to_json(), scale=2)
                    st.download_button("Download Graph as PNG", png_defect,
                                       "defect_analysis_graph.png", "image/png", key='dl_img_defect')
                except Exception as e:
                    st.error(f"Could not generate image: {e}")
            ...
            if total_good_parts_scope == 0:
                st.warning(
                    "Total Good Parts is 0, cannot calculate PPM impact.")
            else:
    ...
    st.write(
        "If you don't have a file, one has been generated as `dummy_data.csv` in your folder.")
